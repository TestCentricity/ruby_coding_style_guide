...
* Never use `do` for multi-line `while` or `until`.
...
* The `and`, `or`, and `not` keywords are banned. Always use `&&`, `||`, and `!` instead.
...
* Indent the `public`, `protected`, and
    ...
    blank line above and below them.
...
* Use `Hash#key?` instead of
    `Hash#has_key?` and `Hash#value?` instead of `Hash#has_value?`. The longer forms are considered deprecated.
